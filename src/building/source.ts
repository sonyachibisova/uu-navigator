...
export interface LabelSpec {
  name: string;
  /**
   * Что подписано. Лестницы и лифты — это якоря: человек видит их глазами
   * и от них считает, где находится, поэтому подписываются они иначе,
   * чем помещения, и проступают раньше.
   */
  kind: 'room' | 'vertical';
  ...
  position: Vec3;
  /**
   * Габарит того, что подписано, в плане: метры по X и по Z.
   *
   * Без него подпись не знает, куда её поставили, и вылезает в стены: размер
   * подписи задан в метрах и от размера помещения не зависел вовсе, а
   * «Технические помещения (5 этаж, западный блок)» одной строкой длиннее
   * любого помещения, в котором такая табличка может висеть. Габарит —
   * это рамка, за которую подпись не выходит; как её соблюсти, решает
   * слой подписей.
   */
  fit: { width: number; depth: number };
  /**
   * Цвет того, что лежит под подписью, в виде `#rrggbb`. Нужен варианту
   * без плашки: цвет цифры считается от него, а палитру знает не источник
   * данных, а сцена — поэтому поле необязательное и заполняется позже.
   */
  underColor?: string;
}
...
/**
 * Вход в здание: точка в плане, через которую человек попадает внутрь.
 *
 * Это не помещение и не связь: у входа нет ни номера, ни площади, зато он
 * есть у любого здания, и именно с него начинается путь человека, пришедшего
 * с улицы. Источник отдаёт его отдельно, потому что знает геометрию оболочки;
 * граф путей пришивает его к ближайшим лестнице и лифту.
 */
export interface EntranceView {
  id: string;
  name: string;
  /** Этаж, на который выходит вход. */
  level: number;
  x: number;
  z: number;
  /** Ширина проёма, метры. */
...
export interface BuildingSource {
  ...
  floors(): FloorView[];
  /**
   * Вход в здание, если он описан геометрией. Метод необязательный: источник,
   * который про вход ничего не знает, его просто не объявляет, и граф путей
   * обходится без узла входа.
   */
