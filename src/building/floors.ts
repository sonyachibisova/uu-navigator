 ...
 * «изолировать этаж». Невыбранные этажи приглушаются независимо от того,
 * выше они выбранного или ниже: срез гасит кольца оболочки, но не интерьеры,
 * иначе здание перестаёт читаться как здание. Ни одно из состояний не делается
  ...
  function setStates(
    ...
    for (const layer of layers) {
      ...
      if (active === null) {
        ...
        // Полное скрытие — только явный режим «изолировать этаж» (инвариант 7
        // правил проекта). Само по себе переключение этажа ничего не прячет.
        target = 0;
      } else {
        // Приглушаются одинаково и нижние этажи, и верхние. Раньше верхние
        // уходили в ноль вместе с кольцами оболочки над срезом — и здание
        // переставало читаться как здание, ровно тот дефект, из-за которого
        // инвариант 7 и появился.
        target = DIMMED;
      }
