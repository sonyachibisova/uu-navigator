...
function main(): void {
  ...
  const ui = createUi(overlayRoot, store, building, {
    ...
    // Стартовый лист и карточка места закрывают низ экрана — здание должно
    // центрироваться над ними, а не во весь холст, часть которого не видна.
    onOcclusionChange(px): void {
      cameraHandle.setBottomInset(px);
    },
    showStep(step): void {
